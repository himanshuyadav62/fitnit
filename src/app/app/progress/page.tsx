    ...
    <Tabs defaultValue="training" className="space-y-6">
      ...
      <TabsContent value="training" className="space-y-6">
        ...
        <Card className="border-white/8"><CardHeader><CardTitle>Recent workout details</CardTitle><CardDescription>Actual date, weekday, plan day, exercises, reps, load and session effort.</CardDescription></CardHeader><CardContent>{analytics.sessions.length ? <div className="divide-y">{analytics.sessions.slice(0, 8).map((session) => {
          ...
          return <div key={session.id} className="py-5 first:pt-0 last:pb-0"><div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-start"><div><div className="flex flex-wrap items-center gap-2"><p className="font-medium">{session.title}</p><Badge variant="outline">Plan day {session.planDay}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{date} · {session.durationMinutes} min{session.effort ? ` · effort ${session.effort}/10` : ""}</p></div><div className="text-sm tabular-nums sm:text-right"><p>{session.totalSets} sets · {session.totalReps} reps</p><p className="text-xs text-muted-foreground">{Math.round(Number(session.volumeKg)).toLocaleString()} kg volume</p></div></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{sessionExercises.map((exercise) => <div key={exercise.planExerciseId} className="flex items-center justify-between rounded-lg bg-muted/35 px-3 py-2 text-xs"><span className="truncate font-medium">{exercise.name}</span><span className="ml-3 shrink-0 text-muted-foreground">{exercise.sets} sets · {exercise.reps} reps · max {Number(exercise.maxWeightKg).toLocaleString()} kg</span></div>)}</div></div>;
