...
import { getActivePlan, getExerciseLibrary, getOpenSessions, getProfile, getUserPlans } from "@/lib/data";
...
export default async function PlanPage() {
  ...
  const [plan, profile, exerciseLibrary, userPlans, openSessions] = await Promise.all([getActivePlan(user.id), getProfile(user.id), getExerciseLibrary(user.id), getUserPlans(user.id), getOpenSessions(user.id)]);
  ...
  const openSessionByWorkout = new Map<string, (typeof openSessions)[number]>();
  for (const session of openSessions) if (!openSessionByWorkout.has(session.planWorkoutId)) openSessionByWorkout.set(session.planWorkoutId, session);
  ...
  return <div className="space-y-8">
    ...
    {openSessions.length > 0 && <Card className="border-primary/30 bg-primary/5"><CardHeader><CardTitle className="text-base">{openSessions.length} workout{openSessions.length === 1 ? "" : "s"} in progress</CardTitle><CardDescription>Resume any unfinished session, or start a different training day below.</CardDescription></CardHeader><CardContent className="flex flex-wrap gap-2">{openSessions.map((session) => <Button key={session.id} variant="outline" asChild><Link href={`/app/workouts/${session.id}`}>Resume {session.title} <ArrowRight /></Link></Button>)}</CardContent></Card>}

    <div className="grid gap-6 lg:grid-cols-2">{plan.workouts.map((workout) => { const openSession = openSessionByWorkout.get(workout.id); return <Card key={workout.id} className="border-white/8">
