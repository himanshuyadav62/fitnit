  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <aside className="space-y-4">
        ...
        {guide?.sources && guide.sources.length > 0 && <Card className="border-border"><CardHeader><CardTitle className="text-base">Learn more</CardTitle><CardDescription>Source videos used to shape this guide. Guidance is summarized and adapted, not copied.</CardDescription></CardHeader><CardContent className="space-y-2">{guide.sources.map((source) => <a key={source.url} href={source.url} target="_blank" rel="noreferrer" className="flex items-start justify-between gap-3 rounded-lg border bg-muted/20 p-3 text-sm transition-colors hover:border-primary/40 hover:text-primary"><span>{source.title}</span><ExternalLink className="mt-0.5 size-4 shrink-0" /></a>)}</CardContent></Card>}
